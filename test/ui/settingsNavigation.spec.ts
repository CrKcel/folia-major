...
// test/ui/settingsNavigation.spec.ts
...
test.describe('settings navigation - wide layout', () => {
    ...
    test('expands a table of contents for the active section only', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
    ...
    test('slots a late-arriving section into its document position', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
    ...
    test('scrolls to a section when its table of contents entry is clicked', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
    ...
    test('follows the scroll position back to the first section', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
    ...
    test('selects the final section once the column bottoms out', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
...
test.describe('settings navigation - narrow layout', () => {
    ...
    test('keeps the chip strip flat, with no grouping and no table of contents', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
    ...
    test('spaces adjacent chips exactly 8px apart', async ({ page }) => {
        await openProbe(page, 'settingsNavigation');
