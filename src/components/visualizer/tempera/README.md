...
用户可以往画布上放自己的图片（立绘/logo/纹理），它们构成一个**图片池**：每个 shot 由 seed 随机取一张（`resolveTemperaShotImage`，相邻 shot 不重复，`layerImageFrequency` 控制出现频率），位置由横纵两个轴的**对齐倾向**加 seed 抖动算出（`resolveTemperaImagePlacement`）。九宫格固定两个轴；纵向随机、横向随机分别只释放一个轴；不限会释放两个轴。翻转、微旋转、尺寸抖动也一并随机——逐张手摆坐标会让「池」失去意义。文件走仓库已有的 `visualizerImageAsset` 存进 IndexedDB，tuning 里只留 id + 横纵倾向 + 大小 + 不透明度，仍然能同步和导入导出。设置面板里只放一条**缩略图条 + 数量**的入口，增删改全部集中在 `TemperaImageLayerDialog`（走仓库的 `ThemedDialog` + `createPortal`，所以在 VisPlayground 和设置弹窗里都不会被祖先 transform 顶偏）——**代价是弹窗挂在 `document.body` 上，拿不到 AppShell 那个 div 上的 `--text-primary/--text-secondary/--text-accent`**：`color: var(--text-primary)` 在这里是无效声明，会回落到 body 的近白色，在 isDaylight 的白底面板上等于字全看不见，所以弹窗内的文字、描边、九宫格标记统一走 `temperaDialogTokens`（按 `ThemedDialog` 的两种面板底色给值，不跟专辑主题走），并在滚动容器上把这三个变量重新声明一遍，`TemperaRangeControl` 这类共用控件才能继续按 `var(--text-*)` 取色——图片池是「选图」，只列文件名等于没法选。**预览资源和实际资源是分开的**：上传时顺手用 canvas 压到最长边 256px 存成 `thumbnail`（webp），`loadTemperaLayerImageThumbnails` 优先取它、缺了才回落到原图；渲染器和 OBS 内联走的仍然是 `loadTemperaLayerImageBlobs` 的原分辨率。立绘常常是印刷级尺寸，池里又能放 16 张，拿原图去喂一排 80px 的预览框等于白解码几千万像素。缩略图 URL 由 `useTemperaLayerImageThumbnails` 按 **id 集合**（而不是数组引用）生成并负责 revoke：拖一次滑块就重读 IndexedDB、重新签发 URL 会让每张缩略图闪一下。弹窗里的所有编辑都只改 `TemperaImageLayerControls` 持有的 **draft**，**关窗时一次性提交**（保存按钮就是关窗）：`handleSetTemperaTuning` 一次写入 = 一次同步 localStorage 序列化 + 一次全局 store 更新，按 pointermove 的频率跑会直接把一个核吃满。编辑期间的反馈由 `TemperaImagePlacementEditor` 承担；它把图片 id 转成稳定 seed，并直接复用 `resolveTemperaImagePlacement`，所以 16:9 画面缩影与 Pixi 使用相同的横纵随机带、尺寸抖动、裁切、翻转和旋转。单张删除与清空全部都只记进 `removedIds`，提交时才真的从 IndexedDB 抹掉；卸载时若弹窗仍开着会补一次提交（effect 依赖必须为空数组，挂 `isDialogOpen` 会让每次正常关闭都提交两遍），否则刚上传的文件会变成没人引用的孤儿记录。**整组的导入导出走独立的 zip**（`src/services/temperaImageArchive.ts`，复用仓库已有的 `fflate`），因为图片字节进不了任何文本通道：短码和同步快照只带 id 和倾向，把图像塞进去会让它们直接爆掉。zip 内是 `meta.json` + `pool.json`（**只有 `layerImages`**）+ `images/<id>.<ext>` 原图像字节，导出时按 `mimeType` 定扩展名；**备份只装图片池本身**——`layerImageDepth` / `layerImageFrequency` 是整池共享、用户此刻就盯在设置面板里的两项，写进备份意味着导入几张立绘会顺手挪掉图层、改掉所有图片（包括它没碰的那些）的出现频率，所以这两个字段既不导出也不导入，导入只动 `layerImages`；导入走 `prepareTemperaLayerImage` 重建记录，所以进来就和拖进来的文件一样（缩略图也重新生成）。已经丢文件的条目不会写进清单——只留 id 不如图的 placement 恢复出来也没画面。id 是跟着文件走的，因此同一份备份连导两次时第二个会重新生成 id，否则它会在 IndexedDB 里把第一次的记录覆盖掉；超过 16 张的尾部会被丢掉并在 toast 里报数——这个数按**可解析的条目**算而不是 manifest 行数（丢文件的行本来就不算），因此只能取循环下标，不能用 `entries.indexOf(entry)`：`indexOf` 按对象引用匹配，只在 `collectEntries` 恰好每行新建一个对象时才对。导入分「替换 / 追加」两种：替换会先弹确认再真的 `clearTemperaLayerImage` 掉旧 blob **以及 `removedIds` 里记着的那些**（不走「提交时才删」那条路——这些文件就算用户不保存也必须消失；只删 `draft.layerImages` 会漏掉本次会话里已被单张删除或清空掉的图，它们已从 draft 里摘走、只剩 `removedIds` 还记着），追加则并进现有池；两者都只换 `layerImages`，层次与频率一律保持用户当前设置。**有写操作在跑时不允许关窗**（`isTemperaPoolWriteLocked`：`adding` 与 `importing`）——**导出不锁，因为它只读 draft、不写任何东西**，中途关窗不会留下半应用状态。大压缩包的解压、读图存图都是异步的，而**这两个操作都在最后一个 await 之后才写 draft**：中途关窗会把「运行前」的池提交进 tuning，那条异步尾巴只继续改一个已经没人看的 draft——追加会留下没人引用的孤儿 blob，替换还可能在落库前把刚提交的 tuning 正指向的文件删掉，**添加则会让「已添加 N 张」的 toast 落在一个刚被保存、并没有变的池上**。所以这两态下保存按钮被禁用、`ThemedDialog` 的 X 与背景点击一起封掉（`closeDisabled` + `closeDisabledTitle`），`commit` 自己也直接返回——最后这条是给「弹窗被卸载」准备的：设置弹窗里按 Esc 会连整个面板一起卸载，不走任何一条关闭路径，此时宁可丢掉 draft（孤儿重新导一次就有了），也不要写出一份指向已被删除文件的 tuning；卸载后那条异步尾巴连 toast 都不再发（`isMountedRef`），否则它会在一个已经回退的池上说「已添加 N 张」。**三条入口共用一个守卫，而且守卫只在回调里**：拖放、底部按钮、隐藏的 file input 都只做一件事——把文件交给 `handleFiles` / `importPool`，所以拒绝第二次运行也只能在这两个回调里；拖放是唯一绕得开按钮 `disabled` 的路径，而原生文件选择框在点开之后仍然开着，用户可以在它等着的时候再拖一个 zip 进来。放在 dialog 里等于每条路径各守一次，漏一条就是两次写入撞进同一个 draft——两者按同一份空闲槽位数准备文件，输的那条多准备的 blob 会被 `.slice` 砍掉，留在 IndexedDB 里没人引用。**繁忙显示在数量那个位置，不是按钮上**：忙碌时 `{{count}} / {{max}}` 换成 spinner + 文案。按钮上转圈等于没有——四个按钮一起变灰时，其中一个悄悄转起来是看不见的；而从原生选择框发起的导入，选择框早已关闭，转的圈不在视线里。数量槽位既不随列表滚动、也从不置灰，是等待期间唯一稳定的落点。这些按钮和「添加图片」一起排在弹窗底部的 **footer 行**、紧挨保存按钮，数量和拖放提示挪到同一行右侧：**footer 本来就不随列表滚动**，不必再为这四个按钮单起一条固定行（那是另一种把同类操作拆成两排的做法）；池里放满 16 张时列表有好几屏高，跟着滚的上传控件意味着每加一张都要先滚回顶部。拖放落点仍是整块区域，zip 按追加导入处理，落点提示留在滚动区下方的虚线框里。zip 的压缩与解压走 `fflate` 的异步 `zip`/`unzip`（在工作线程里跑，池里最多 16 张印刷级尺寸的图，在主线程 deflate 几十 MB 会让身后正在播的歌词动画明显顿一下）。导入导出有** toast 结果**：成功和失败的条数（`skipped` 是清单里缺字节的行、`truncated` 是被上限挡下的行，两者互斥所以可以相加；明细只进 console）。失败走 `temperaPoolImportFailed` / `temperaPoolExportFailed` 并在 console 打错误。导出文件名是 `t('options.temperaExportBaseName')` 加本地日期（`formatLocalDateStamp`）。
